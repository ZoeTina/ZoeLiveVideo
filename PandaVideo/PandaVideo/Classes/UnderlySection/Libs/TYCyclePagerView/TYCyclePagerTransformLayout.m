...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    if (self.layout.layoutType == TYCyclePagerTransformLayoutLinear) {
        ...
        for (UICollectionViewLayoutAttributes* attributes in array) {
            ...
            if (CGRectIntersectsRect(attributes.frame, rect)) {
                ...
                CGFloat tempScaleFactor = ScaleFactor;
                if (array.count == 1) {
                    tempScaleFactor = 0;
                }
                
                if (ABS(distance) < ActiveDistance) {
                    CGFloat zoom = 1 + tempScaleFactor*(1 - ABS(normalizedDistance));
    ...
    if (_layout.adjustSpacingWhenScroling) {
        ...
        switch (direction) {
            case TYTransformLayoutItemLeft:
                translate = 3.15 * attributes.size.width*(1-scale)/2;
                break;

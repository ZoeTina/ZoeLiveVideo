...
@interface PVBarrageView ()

@property (nonatomic, strong) CADisplayLink *displayLink;

@property (nonatomic, strong) UIView<PVBarrageItemProtocol> *nextItem;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _barrageDistance = 16;
        _state = PVBarrageAnimationState_stop;
...
- (BOOL)startBarrage {
    if (_state == PVBarrageAnimationState_stop) {
        _state = PVBarrageAnimationState_playing;
        [self displayLink];
        return YES;
    }
    return NO;
...
- (BOOL)pauseBarrage {
    if (_state == PVBarrageAnimationState_playing) {
        _state = PVBarrageAnimationState_pause;
...
- (BOOL)resumeBarrage {
    if (_state == PVBarrageAnimationState_pause) {
        _state = PVBarrageAnimationState_playing;
        [self displayLink];
        return YES;
    }
...
- (BOOL)stopBarrage {
    if (_state == PVBarrageAnimationState_pause || _state == PVBarrageAnimationState_playing) {
        _state = PVBarrageAnimationState_stop;
        [self destoryDisplayLink];
        ...
        for (NSUInteger i = 0; i < _playQueue.count; i ++) {
            ...
            for (NSInteger j = 0; j < subQueue.count; j ++) {
                UIView<PVBarrageItemProtocol> *item = subQueue[j];
...
- (UIView<PVBarrageItemProtocol> *)dequeueReusableItem {
    UIView<PVBarrageItemProtocol> *item = [_reusePool anyObject];
    if (item) {
...
- (void)update:(CADisplayLink *)link {
    ...
    UIView<PVBarrageItemProtocol> *item = _nextItem;
    if (!item) {
...
- (void)itemFinishPlay:(UIView<PVBarrageItemProtocol> *)item {
    [item removeFromSuperview];
...
- (BOOL)addItemToPlayQueue:(UIView<PVBarrageItemProtocol> *)item {
    BOOL hadAdded = NO;
    if (!item) {
        return hadAdded;
    }
    ...
    for (NSUInteger i = 0; i < _playQueue.count; i ++) {
        NSArray *subQueue = _playQueue[i];
        UIView<PVBarrageItemProtocol> *curItem = [subQueue lastObject];
...
/// 弹幕播放完毕还需要多少时间
- (NSTimeInterval)itemFinishedPlayingTime:(UIView<PVBarrageItemProtocol> *)item appendDistance:(NSInteger)append {
    if (!item) {
        return 0;
    }
    if (item.right < 0) {
...
/// 初始化弹幕的配置
- (void)initItem:(UIView<PVBarrageItemProtocol> *)item {
    item.left = [self barrageBeginX];
    item.y = 0;
    item.curDistance = 0;
}

- (void)configItem:(UIView<PVBarrageItemProtocol> *)item inSubQueueIndex:(NSUInteger)index {

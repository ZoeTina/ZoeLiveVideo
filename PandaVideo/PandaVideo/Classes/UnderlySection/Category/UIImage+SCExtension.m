...
+ (UIImage *)sc_singleDotImageWithColor:(UIColor *)color {
 ...
 * JPG 图片不支持透明
 */
- (UIImage *)sc_avatarImage:(CGSize)size
...
+ (UIImage *)getThumbImage:(UIImage *)image  size:(CGSize)size{
    ...
    if (nil == image) {
        ...
        CGFloat scaleBy = (size.width*2)/image.size.width;
        scaleBy = scaleBy > 1?1:scaleBy;
        scaleBy = (scaleBy <= 0)?0.1:scaleBy;

//
//  PVInterractionCurrentListViewController.m
//  PandaVideo
//
...
#import "PVInterractionCurrentListViewController.h"
#import "PVBroadcastLiveCell.h"
#import "PVCurrentListModel.h"

@class PVCurrentListData,PVRankingList,LZUser;

static NSString *CellIdentifier = @"PVInterractionCurrentListViewController";
@interface PVInterractionCurrentListViewController ()
...
@implementation PVInterractionCurrentListViewController
- (id)initDictionary:(NSDictionary *)dictionary{
    
    if ( self = [super init] ){
        
        _liveId  = dictionary[@"liveId"];//@"4f21d6fee8c64209907ac2ee2ca5c742";//
    }
    return self;
}
...
//        if(self.currentIndex == self.pageAllIndex)return;
    }];}

- (void) loadData{
    NSDictionary *dictionary = @{@"liveId":_liveId};
    [PVNetTool postDataWithParams:dictionary url:@"getRankingList" success:^(id result) {
        YYLog(@"result -- %@",result);
        if (result != nil) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PVRankingList *listModel = self.itemModelArray[indexPath.row];
    cell.listDataModel = listModel;

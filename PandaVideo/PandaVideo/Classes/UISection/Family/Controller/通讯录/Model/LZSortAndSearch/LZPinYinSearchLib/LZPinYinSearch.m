...
+(NSArray *)searchWithOriginalArray:(NSArray *)originalArray andSearchText:(NSString *)searchText andSearchByPropertyName:(NSString *)propertyName{
    ...
    if(originalArray.count <= 0){
        ...
        if ([object isKindOfClass:[NSString class]]) {
            ...
            if (!isExit) {
                NSString *string = [NSString stringWithFormat:@"数据源中的字典没有你指定的key:%@",propertyName];
                YYLog(@"%@",string);
                return originalArray;
            }
        } else {
            ...
            if (!isExit) {
                NSString *string = [NSString stringWithFormat:@"数据源中的Model没有你指定的属性:%@",propertyName];
                YYLog(@"%@",string);
                return originalArray;
            }
    ...
    if (searchText.length>0&&![LZChineseInclude isIncludeChineseInString:searchText]) {
        for (int i=0; i<originalArray.count; i++) {
            ...
            if ([LZChineseInclude isIncludeChineseInString:tempString]) {
                NSString *tempPinYinStr = [LZPinYinForObjc chineseConvertToPinYin:tempString];
                ...
                NSString *tempPinYinHeadStr = [LZPinYinForObjc chineseConvertToPinYinHead:tempString];
    ...
    } else if (searchText.length>0&&[LZChineseInclude isIncludeChineseInString:searchText]) {

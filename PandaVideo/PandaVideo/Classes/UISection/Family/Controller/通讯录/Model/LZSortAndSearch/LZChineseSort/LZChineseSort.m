...
+ (NSMutableArray *)getAllValuesFromDictionary:(NSDictionary *)dictionary {
    NSMutableArray *valuesAry = [NSMutableArray new];
    NSArray *keyAry = [self sortForStringAry:[dictionary allKeys]];
...
//        [UIAlertController showAlertViewWithTitle:@"提示" Message:@"数据源不能为空" BtnTitles:@[@"确定"] ClickBtn:nil];
    ...
    if ([objc isKindOfClass:[NSString class]]) {
        ...
        for (NSString *str in ary){
            LZChineseSort *sortString = [LZChineseSort new];
...
//                    [UIAlertController showAlertViewWithTitle:@"提示" Message:[NSString stringWithFormat:@"数据源中的Model没有你指定的属性:%@",name] BtnTitles:@[@"确定"] ClickBtn:nil];
    ...
    NSString *itemString;
    for (LZChineseSort *sort in sortAry){
...
+ (NSMutableArray *)sortAsInitialWithArray:(NSArray *)ary {
    ...
    for (NSInteger index = 0; index < ary.count; index++) {
        LZChineseSort *sortString = ary[index];
        sortString.englishString = [LZChineseSort transform:sortString.string];
        ...
        if (sortString.string.length) {
            //得到字符串首个字符
            header = [sortString.string substringToIndex:1];
        }

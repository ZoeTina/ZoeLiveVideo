...
@property (nonatomic, assign) SCIndicatorType indicatorType;


@end

@implementation SCSegmentTitleView
...
- (void)initWithProperty
{
    ...
    self.titleSelectColor = [UIColor sc_colorWithHex:0x2AB4E4];
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [self getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [self getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
    if (totalBtnWidth <= CGRectGetWidth(self.bounds)) {//不能滑动
        if (self.itemBtnArr.count < 4) {
            CGFloat currentX = 0;
            for (int idx = 0; idx < self.titlesArr.count; idx++) {
                UIButton *btn = self.itemBtnArr[idx];
                titleFont = btn.isSelected?_titleSelectFont:_titleFont;
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [self getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case SCIndicatorTypeDefault:
                ...
                break;
            case SCIndicatorTypeEqualTitle:
...
-(void)setTempSelectIndex:(NSInteger)tempSelectIndex{
    _tempSelectIndex = tempSelectIndex;
    _selectIndex = tempSelectIndex;
}
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(SCSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate SCSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
- (CGFloat)getWidthWithString:(NSString *)string font:(UIFont *)font {
    ...
    if (selectBtn.isSelected) {
        return  [UILabel messageBodyText:string andBoldSystemFontOfSize:[_titleSelectFont pointSize] andLabelwith:0 andLabelheight:0].width;
    }else{

...
- (void)sd_setImageWithURL:(nullable NSURL *)url forState:(UIControlState)state {
    NSString* tempUrl = [url.absoluteString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [self sd_setImageWithURL:[NSURL URLWithString:tempUrl] forState:state placeholderImage:nil options:0 completed:nil];
}
